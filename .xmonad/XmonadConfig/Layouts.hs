...
myScratchpads =
  [ NS "alacritty"  "XMODIFIERS= alacritty --title 'Alacritty(floating)' -o background_opacity=0.97" (title =? "Alacritty(floating)")
        myTermFloating
  ...
  , NS "discord"    "discord" (className =? "discord")
        myFullFloating
  , NS "ncmcpp"     "XMODIFIERS= alacritty -e ncmcpp --title ncmcpp -h mopidy.local" (title =? "ncmpcpp")
        myFullFloating
  , NS "mpsyt"      "XMODIFIERS= alacritty --title mpsyt -e $HOME/.local/bin/mpsyt" (title =? "mpsyt")
        myFullFloating
  , NS "slack"      "slack" (className =? "Slack")
        myFullFloating
  , NS "sidekick"   "sidekick-browser-stable" (className =? "Sidekick-browser")
        myFullFloating
  ]
